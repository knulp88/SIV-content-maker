...
export default {
  ...
  mounted() {
    ...
    if (this.$route.path === "/link-area") {
      ...
      if(this.$ls.get('linkAreaData')){
        for(const [idx, val] of Object.entries(this.$ls.get('linkAreaData'))){
          Object.assign(this.areaData[idx].linkArea, val.linkArea);
  ...
  methods: {
    // // vuex state update
    updateState(){
      this.$ls.set('linkAreaData', this.areaData);
    },
    dbclick(evt, idx) {
